...
public class AddLicenseFeatureServlet extends HttpServlet
{

   public AddLicenseFeatureServlet()
   {
   }
      ...
      public void setResult(int result)
      {
         ...
         if(result == ResultMessage.Success)
         {
            ...
            this.Report.setMessage(
               "The license feature has been successfully added.");
         }
         else if(result == ResultMessage.Exists)
         {
            this.Report.setType(ResultMessage.ErrorMessage);
            this.Report.setMessage(
               "The license feature already exists.");
         }
         else if(result == ResultMessage.Failure)
         {
            this.Report.setType(ResultMessage.ErrorMessage);
            this.Report.setMessage(
               "Unable to add the license feature.");
         }
         else if(result == ResultMessage.NotAuthorized)
         {
            this.Report.setType(ResultMessage.ErrorMessage);
            this.Report.setMessage(
               "You are not authorized to add a license feature.<br />" +
      ...
      throws IOException, ServletException
   {
      ...
      AddLicenseFeatureServlet.Message message = new AddLicenseFeatureServlet.Message(
      ...
      String page = "/licensefeatures.jsp?license=" + licenseId;
      ...
      try
      {
         ...
         try
         {
            ...
            if(licenses.isFeatureUsed(licenseId, featureId))
               message.setResult(ResultMessage.Exists);
            else
            {
               // Add a new license feature.
               licenses.database.statement.executeUpdate(
                  "INSERT INTO licensefeatures (featureid, licenseid, autoadd) " +
                  "VALUES (" + featureId + ", " + licenseId + ", 0)");
