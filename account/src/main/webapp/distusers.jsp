...
<title>Coreform - Distributor Clients</title>
...
<% String idString = request.getParameter("user");
   String currentPage = "distusers";
   if(idString == null || idString.isEmpty())
     currentPage = "distusers.jsp"; %>
...
<% if(isLoggedIn)
   {
     ...
     {
       csimsoft.DistributorUsers userList = new csimsoft.DistributorUsers();
       try
       {
         int userId = 0;
         if(isDistributor)
          ...
          <td colspan="3">
            <ul class="tool-bar">
              <li class="tool-button">
                <a href="selectlicense.jsp?next=licensesearch.jsp&seltype=both&dist=<%= userId %>">
                <img src="./images/icon-license.png" width="20" height="20" />
                License Search</a>
              </li>
              <li class="tool-button">
                 <a href="selectproduct.jsp?next=productsearch&seltype=files&root=<%= userList.getFolder() %>&dist=<%= userId %>">
                <img src="./images/icon-file.png" width="20" height="20" />
                Product Search</a>
              </li>
              <li class="tool-button">
                 <a href="ordersearch.jsp?root=<%= userList.getFolder() %>&dist=<%= userId %>">
...
<%       } %>
        ...
        <tr>
          <td class="title-cell" colspan="3">
            Clients
            <form action="distusers.jsp" method="GET" name="searchForm"
...
<%           } %>
          <td class="list-row">
            <a href="distclient.jsp?user=<%= client.getId() %>">
