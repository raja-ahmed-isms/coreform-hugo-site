...
<title>Coreform - License Features</title>
  ...
  <div class="main">
    ...
    <div class="navleft">
      <jsp:include page="navigation.jsp" flush="true" >
        <jsp:param name="current" value="licensefeatures.jsp" />
      </jsp:include>
    </div>
...
<% if(isLoggedIn)
   {
     if(isAdmin)
     {
       ...
       try
       {
         ...
         {
           csimsoft.LicenseFeatures features = new csimsoft.LicenseFeatures();
           features.setFeatures(licenses, licenseId); %>

    <div class="content">
      <form action="changedefaultfeatures" method="POST" name="editfeaturesform">
      ...
      <table border="0" cellpadding="6" cellspacing="0" width="100%">
          ...
          <td colspan="4">
            ...
            <ul class="tool-bar">
              <li class="tool-button">
                <a href="selectfeature.jsp?next=newlicensefeature&license=<%= licenseId %>">
        ...
        <tr>
          <td class="title-cell" colspan="4">License Features</td>
        </tr>
<%         if(features.isEmpty())
           { %>
        <tr>
          <td class="list-row" colspan="4">
            There are no features in the database for this license.
          </td>
        </tr>
<%         }
           else
...
<%           for(int i = 0; i < features.getFeatureCount(); ++i)
             {
               csimsoft.LicenseFeatures.Feature feature = features.getFeature(i); %>
        <tr>
          <td class="list-icon">
            <a href="javascript:sendDeleteFeatureForm(<%= feature.Id %>, '<%= feature.getJsName() %>')"
              ><img src="./images/icon-delete.jpg" width="20" height="20" /></a>
          </td>
          <td class="list-row"><%= feature.Name %></td>
          <td class="list-row"><%= feature.Key %></td>
          <td class="list-row">
            <input type="checkbox" name="<%= feature.Id %>"
            value="<%= feature.Id %>" <%= feature.getChecked() %> />
          </td>
        </tr>
<%           } %>
        <tr>
...
<%         } %>
      ...
      <form action="removelicensefeature" method="POST" name="deletefeatureform">
