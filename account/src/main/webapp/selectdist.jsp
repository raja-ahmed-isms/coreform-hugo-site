...
<title>Coreform - Select Distributor</title>
...
function sendSelectDistForm(groupid)
{
  document.selectdistform.groupid.value = groupid
  document.selectdistform.submit()
}
</script>
<% } %>
  ...
  <div class="main">
    ...
    <div class="navleft">
      <jsp:include page="navigation.jsp" flush="true" >
        <jsp:param name="current" value="selectdist.jsp" />
      </jsp:include>
    </div>
...
<% if(isLoggedIn)
   {
     if(isAdmin)
     {
       csimsoft.DistributorList distributors = new csimsoft.DistributorList();
       ...
       distributors.setExtraParameters(extras.toString());
       params.removeParameter("next");
       distributors.pages.setPage(request.getParameter("page"));
       try
       {
...
<%       if(distributors.pages.getPages() > 1)
         { %>
        <tr>
          <td colspan="3">
            <%= distributors.pages.getMenu() %>
          </td>
        </tr>
<%       } %>
        <tr>
          <td class="title-cell" colspan="3">Select a Distributor</td>
        </tr>
        <tr>
          <td class="list-icon">
          ...
          <td class="list-row">
            <a href="javascript:sendSelectDistForm('0')">No Distributor</a>
          </td>
          <td class="list-row">&nbsp;</td>
        </tr>
<%       while(distributors.hasNext())
         {
           csimsoft.UserProfileInfo info = distributors.getNext(); %>
        <tr>
          <td class="list-icon">
            <img src="./images/icon-user.png" width="20" height="20" />
          </td>
          <td class="list-row">
            <a href="javascript:sendSelectDistForm('<%= info.getGroupId() %>')">
            <%= info.getFullName() %></a>
          </td>
          <td class="list-row">
